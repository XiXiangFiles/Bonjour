...
mdns.on('query',function(res){
	let promise=new Promise(function(resolve,reject){
		...
		if(res.questions.map((element)=>{return element.type}).includes('PTR')){
			if(res.questions.map(function(element){if(element.type=='PTR'){return element.name}}).includes(config.Service[0])){
				let obj={};
				obj.name=config.Service[0];
				obj.type='PTR';
				obj.ttl=120;
				obj.data=config.Instance+'.'+config.Service[0];
				answers.push(obj);
		...
		if(res.questions.map((element)=>{return element.type}).includes('SRV')){
			if(res.questions.map(function(element){if(element.type=='SRV'){return element.name}}).includes(config.Instance+'.'+config.Service[0])){
				let obj={};
				obj.name=config.Instance+'.'+config.Service[0];
				obj.type='SRV';
				obj.ttl=120;
				obj.data={};
		...
		if(res.questions.map((element)=>{return element.type}).includes('TXT')){
			if(res.questions.map(function(element){if(element.type=='TXT'){return element.name}}).includes(config.Instance+'.'+config.Service[0])){
				let obj={};
				obj.name=config.Instance+'.'+config.Service[0];
				obj.type='TXT';
				obj.ttl=120;
